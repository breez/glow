...
void main() {
  ...
  group('formatTxid', () {
    final shortTxids = ['short', '1234567890123456'];
    for (final txid in shortTxids) {
    ...
    test('returns shortened txid when length > 16', () {
      const longTxid = '1234567890abcdefghijklmnopqrstuvwxyz';
      final result = claimer.formatTxid(longTxid);
  ...
  group('hasError', () {
    test('true when deposit has claim error', () {
      final deposit = _createMockDeposit(claimError: DepositClaimError.missingUtxo(tx: 'test', vout: 0));
      expect(claimer.hasError(deposit), true);
    });
    test('false when deposit has no claim error', () {
      final deposit = _createMockDeposit();
  ...
  group('hasRefund', () {
    test('true when deposit has refund transaction', () {
      final deposit = _createMockDeposit(refundTx: 'refund_tx_data');
  ...
  group('formatError', () {
    test('depositClaimFeeExceeded with fixed max fee', () {
      final error = DepositClaimError.depositClaimFeeExceeded(
        tx: 'test',
        vout: 0,
        maxFee: Fee.fixed(amount: BigInt.from(1000)),
        actualFee: BigInt.from(1500),
      );
      final result = claimer.formatError(error);
    ...
    test('missingUtxo error', () {
      final error = DepositClaimError.missingUtxo(tx: 'test', vout: 0);
      expect(claimer.formatError(error), 'Transaction output not found on chain');
    });
    test('generic error', () {
      final error = DepositClaimError.generic(message: 'Custom error message');
  ...
  group('formatMaxFee', () {
    test('fixed max fee', () {
      final maxFee = Fee.fixed(amount: BigInt.from(2000));
      expect(claimer.formatMaxFee(maxFee), '2000 sats');
    });
    test('rate max fee', () {
      final maxFee = Fee.rate(satPerVbyte: BigInt.from(15));
